...
function GracePeriodContentBody(): React.ReactNode {
  return (
    <>
      <Text>
        {t('An update to our Consumer Terms and Privacy Policy will take effect on ')}
        <Text bold>October 8, 2025</Text>
        {t('. You can accept the updated terms today.')}
      </Text>

      <Box flexDirection="column">
        <Text>{t("What's changing?")}</Text>

        <Box paddingLeft={1}>
          <Text>
            <Text>· </Text>
            <Text bold>{t('You can help improve Claude ')}</Text>
            <Text>
              {t(
        ...
        <Box paddingLeft={1}>
          <Text>
            <Text>· </Text>
            <Text bold>{t('Updates to data retention ')}</Text>
            <Text>
              {t(
...
function PostGracePeriodContentBody(): React.ReactNode {
  return (
    <>
      <Text>{t("We've updated our Consumer Terms and Privacy Policy.")}</Text>

      <Box flexDirection="column" gap={1}>
        <Text>{t("What's changing?")}</Text>

        <Box flexDirection="column">
          <Text bold>{t('Help improve Claude')}</Text>
          <Text>
            {t(
        ...
        <Box flexDirection="column">
          <Text bold>{t('How this affects data retention')}</Text>
          <Text>
            {t(
...
export function GroveDialog({ showIfAlreadyViewed, location, onDone }: Props): React.ReactNode {
  ...
  return (
    ...
    >
      ...
      <Box flexDirection="column" gap={1}>
        <Box flexDirection="column">
          <Text bold>{t("Please select how you'd like to continue")}</Text>
          <Text>{t('Your choice takes effect immediately upon confirmation.')}</Text>
        </Box>
...
export function PrivacySettingsDialog({
  ...
  return (
    ...
    >
      <Text>
        {t('Review and manage your privacy settings at ')}
        <Link url={'https://claude.ai/settings/data-privacy-controls'}></Link>
      </Text>
