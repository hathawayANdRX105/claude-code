...
export function formatReviewStageCounts(
  ...
  if (!stage) return t('{{n}} found · {{m}} verified', { n: found, m: verified });
  if (stage === 'synthesizing') {
    const parts = [t('{{n}} verified', { n: verified })];
    if (refuted > 0) parts.push(t('{{n}} refuted', { n: refuted }));
    parts.push(t('deduping'));
    return parts.join(' · ');
  }
  if (stage === 'verifying') {
    const parts = [t('{{n}} found', { n: found }), t('{{n}} verified', { n: verified })];
    if (refuted > 0) parts.push(t('{{n}} refuted', { n: refuted }));
    return parts.join(' · ');
  }
  // stage === 'finding'
  return found > 0 ? t('{{n}} found', { n: found }) : t('finding');
}
