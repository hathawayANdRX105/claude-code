...
export function MCPRemoteServerMenu({
  ...
  const handleClaudeAIAuthComplete = React.useCallback(async () => {
    ...
    try {
      ...
      if (success) {
        onComplete?.(t('Authentication successful. Connected to {{name}}.', { name: server.name }));
  ...
  const handleClaudeAIClearAuthComplete = React.useCallback(async () => {
    ...
    onComplete?.(t('Disconnected from {{name}}.', { name: server.name }));
      ...
      if (server.config) {
        ...
        if (result.client.type === 'connected') {
          const message = isEffectivelyAuthenticated
            ? t('Authentication successful. Reconnected to {{name}}.', { name: server.name })
            : t('Authentication successful. Connected to {{name}}.', { name: server.name });
          onComplete?.(message);
        } else if (result.client.type === 'needs-auth') {
          onComplete?.(
            t(
  ...
  const handleClearAuth = async () => {
    ...
    if (server.config) {
      ...
      onComplete?.(t('Authentication cleared for {{name}}.', { name: server.name }));
    }
  };
  ...
  if (isAuthenticating) {
    ...
    return (
      <Box flexDirection="column" gap={1} padding={1}>
        <Text color="claude">{t('Authenticating with {{name}}…', { name: server.name })}</Text>
        <Box>
          <Spinner />
          <Text>{authCopy}</Text>
        </Box>
  ...
  if (isClaudeAIClearingAuth) {
    return (
      <Box flexDirection="column" gap={1} padding={1}>
        <Text color="claude">{t('Clear authentication for {{name}}', { name: server.name })}</Text>
  ...
  if (isReconnecting) {
    return (
      <Box flexDirection="column" gap={1} padding={1}>
        <Text color="text">
          {t('Connecting to')} <Text bold>{server.name}</Text>…
        </Text>
        <Box>
          <Spinner />
