...
export function BrowseMarketplace({
      ...
      try {
        ...
        if (errorResult) {
          if (errorResult.type === 'warning') {
            setWarning(errorResult.message + t('. Showing available marketplaces.'));
          } else {
        ...
        if (targetPlugin) {
          ...
          if (foundPlugin && foundMarketplace) {
            ...
            if (globallyInstalled) {
              setError(
                t("Plugin '{{plugin}}' is already installed globally. Use '/plugin' to manage existing plugins.", {
                  plugin: pluginId,
                }),
              );
            } else {
            ...
            setError(t('Plugin "{{plugin}}" not found in any marketplace', { plugin: targetPlugin }));
          }
          ...
          if (marketplaceExists) {
            ...
            setError(t('Marketplace "{{name}}" not found', { name: targetMarketplace }));
          }
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    async function loadPluginsForMarketplace(marketplaceName: string) {
      ...
      try {
        ...
        if (cancelled) return;
        if (!marketplace) {
          throw new Error(t('Failed to load marketplace: {{name}}', { name: marketplaceName }));
        }
  ...
  const installSelectedPlugins = async () => {
    ...
    if (failureCount === 0) {
      // All succeeded
      const message = t('✓ Installed {{count}} {{unit}}. Run /reload-plugins to activate.', {
        count: successCount,
        unit: t(plural(successCount, 'plugin')),
      });

      setResult(message);
    } else if (successCount === 0) {
      // All failed - show error with reasons
      setError(t('Failed to install: {{details}}', { details: formatFailureDetails(newFailedPlugins, true) }));
    } else {
      // Mixed results - show partial success
      const message = t(
        '✓ Installed {{count}} of {{total}} plugins. Failed: {{details}}. Run /reload-plugins to activate successfully installed plugins.',
        {
          count: successCount,
    ...
    return (
      <PluginOptionsFlow
        ...
        onDone={(outcome, detail) => {
          switch (outcome) {
            case 'configured':
              finish(t('✓ Installed and configured {{name}}. Run /reload-plugins to apply.', { name: plugin.name }));
              break;
            case 'skipped':
              finish(t('✓ Installed {{name}}. Run /reload-plugins to apply.', { name: plugin.name }));
              break;
            case 'error':
              finish(t('Installed but failed to save config: {{error}}', { error: detail }));
              break;
          }
        }}
      />
            ...
            <Box marginLeft={2}>
              <Text dimColor>
                {t('{{count}} {{unit}} available', {
                  ...
                  unit: t(plural(marketplace.totalPlugins, 'plugin')),
                })}
                {marketplace.installedCount > 0 &&
                  t(' · {{count}} already installed', { count: marketplace.installedCount })}
      ...
      <Box flexDirection="column">
        ...
        <Box flexDirection="column" marginBottom={1}>
          ...
          {selectedPlugin.entry.version && (
            <Text dimColor>{t('Version: {{version}}', { version: selectedPlugin.entry.version })}</Text>
          )}
        ...
        <Box flexDirection="column" marginBottom={1}>
          ...
          {selectedPlugin.entry.hooks && (
            <Text dimColor>
              {t('· Hooks: {{hooks}}', { hooks: Object.keys(selectedPlugin.entry.hooks).join(', ') })}
            </Text>
          )}
        ...
        {installError && (
          <Box marginBottom={1}>
            <Text color="error">{t('Error: {{error}}', { error: installError })}</Text>
          </Box>
        )}
            ...
            <Box>
              ...
              <Text color={plugin.isInstalled ? 'success' : undefined}>
                ...
                {installCounts && selectedMarketplace === OFFICIAL_MARKETPLACE_NAME && (
                  <Text dimColor>
                    {' · '}
                    {t('{{count}} installs', { count: formatInstallCount(installCounts.get(plugin.pluginId) ?? 0) })}
                  </Text>
                )}
              </Text>
            </Box>
