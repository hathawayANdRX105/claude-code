...
- **单文件产物 (compile)**: `scripts/compile.ts` 逐平台循环（**必须单数** `compile: { target }`，复数被 Bun 静默忽略）产出 `dist/ccb-*`，默认启用 JSC bytecode 预编译（`format: 'esm'` + `bytecode: true`，Bun 1.4+）——`--version` 1.16s→0.20s，代价体积 +53%。bundle 变更后必须过 `--check-commands` 运行时冒烟（上游 #27955 风险，见 DEV-LOG）。
