...
| **自定义模型供应商**        | OpenAI/Anthropic/Gemini/Grok 兼容（`/login`）；`providers.json` 的 `models` 列表可整体替换 `/model` 内置行 | [文档](https://ccb.agent-aura.top/docs/features/all-features-guide) · [自定义模型配置](./docs/extensibility/custom-providers.mdx)   |
